...
tipo_carro = input('Qual é o tipo do carro? (BMW, AUDI, FIAT ou FERRARI) ').upper()
tipo_combustivel = input('Qual é o tipo de combustível? (gasolina ou alcool) ').lower()
qtd_portas = int(input('Quantas portas o carro tem? '))
qtd_pessoas = int(input('Quantas pessoas vão viajar no carro? '))
tem_bagageiro = input('O carro tem bagageiro? (sim ou nao) ').lower()
